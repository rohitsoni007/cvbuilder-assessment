...
export default function Project({ activeStep, handleBack, handleNext }) {
  ...
  useEffect(() => {
    if (selectedResume) {
      setInitialValues((prev) => {
        return {
          ...prev,
          projects: selectedResume?.projects?.length ? selectedResume?.projects : [
            {
              title: "",
              size: "",
              duration: "",
  ...
  const resumeValidationSchema = Yup.object().shape({
    ...
    .of(
      Yup.object().shape({
        ...
        duration: Yup.number().max(100).required("Duration is required"),
        technologies: Yup.string().max(255).required("Technologies is required"),
        description: Yup.string().max(255).required("Description is required"),
        size: Yup.number().max(100).required("Size is required"),
      })
    ),
  });
